...
function register_js_scripts() {
	if (!is_admin()) {
		...
		if ( 
			...
			$cmsmasters_gmap_api_key = (isset($cmsmasters_option[CMSMASTERS_SHORTNAME . '_gmap_api_key']) ? $cmsmasters_option[CMSMASTERS_SHORTNAME . '_gmap_api_key'] : '');
			
			wp_enqueue_script('gMapAPI', '//maps.googleapis.com/maps/api/js?key=' . $cmsmasters_gmap_api_key, array('jquery'), '1.0.0', true);
		...
		if (CMSMASTERS_WOOCOMMERCE) {
			...
			$cmsmasters_image_sizes = cmsmasters_image_thumbnail_list();
			
			wp_localize_script('cmsmasters-woo-script', 'cmsmasters_woo_script', array( 
				'currency_symbol' => 			$currency_symbol, 
				'thumbnail_image_width' => 		$cmsmasters_image_sizes['small-thumb']['width'], 
				'thumbnail_image_height' => 	$cmsmasters_image_sizes['small-thumb']['height'] 
			));
		}
	}
...
add_action('enqueue_block_editor_assets', 'cmsmasters_theme_google_fonts_generate');
...
/* Check Row p Wrapper */
function cmsmasters_rowcheck($content) {
...
/* Get Singular ID */
function cmsmasters_get_singular_id() {
	$id = false;
...
function cmsmasters_theme_bg_styles() {
	...
	$cmsmasters_page_id = cmsmasters_get_singular_id();
...
function cmsmasters_theme_page_layout_scheme() {
	...
	} elseif (CMSMASTERS_EVENTS_CALENDAR && tribe_is_event_query() && isset($cmsmasters_option[CMSMASTERS_SHORTNAME . '_events_layout'])) {
...
/* Return content */
function cmsmasters_return_content($content) {
	return $content;
}
...
add_filter('widget_text_content', 'cmsmasters_global_shortcodes_styles_move', 11, 3);


function cmsmasters_row_checker($data) {
	$out = cmsmasters_rowcheck($data);
	
	return $out;
}

add_filter('the_content', 'cmsmasters_row_checker', 10, 2);
...
function cmsmasters_ajax_export_settings_callback() {
	...
	if( wp_verify_nonce($nonce, 'cmsmasters_ajax_export_settings-nonce') ){
		...
		echo cmsmasters_return_content($result);
