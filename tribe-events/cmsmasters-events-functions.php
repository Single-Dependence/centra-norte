...
function cmsmasters_tribe_update_default_options() {
	...
	$new_global_elements = array(
		'event_title_color' => cmsmasters_theme_color( 'heading' ),
		'event_date_time_color' => cmsmasters_theme_color( 'color' ),
		'link_color' => cmsmasters_theme_color( 'link' ),
		'background_color_choice' => 'transparent',
		'background_color' => 'transparent',
		'accent_color' => cmsmasters_theme_color( 'hover' ),
		'map_pin' => '',
	);
	...
	$new_single_event = array(
		...
		'post_title_color' => cmsmasters_theme_color( 'heading' ),
	...
	$new_tec_events_bar = array(
		'events_bar_text_color' => cmsmasters_theme_color( 'heading' ),
		'find_events_button_text_color' => cmsmasters_theme_color( 'bg' ),
		'events_bar_icon_color_choice' => 'default',
		'events_bar_icon_color' => cmsmasters_theme_color( 'heading' ),
		'find_events_button_color_choice' => 'custom',
		'find_events_button_color' => cmsmasters_theme_color( 'bg' ),
		'events_bar_background_color_choice' => 'default',
		'events_bar_background_color' => cmsmasters_theme_color( 'bg' ),
		'view_selector_background_color_choice' => 'default',
		'view_selector_background_color' => cmsmasters_theme_color( 'bg' ),
		'events_bar_border_color_choice' => 'default',
		'events_bar_border_color' => cmsmasters_theme_color( 'border' ),
	);
	...
	$new_month_view = array(
		'days_of_week_color' => cmsmasters_theme_color( 'heading' ),
		'date_marker_color' => cmsmasters_theme_color( 'heading' ),
		'multiday_event_bar_color_choice' => 'default',
		'multiday_event_bar_color' => '#334aff',
		'grid_lines_color' => cmsmasters_theme_color( 'border' ),
		'grid_hover_color' => cmsmasters_theme_color( 'border' ),
		'grid_background_color_choice' => 'transparent',
		'grid_background_color' => cmsmasters_theme_color( 'color' ),
...
add_action( 'init', 'cmsmasters_tribe_update_default_options' );


/* Theme colors for updating default options in The Events Calendar plugin. */
function cmsmasters_theme_color( $color ) {
	$cmsmasters_option = cmsmasters_get_global_options();

	$colors = array(
		'color' => $cmsmasters_option['mall_default_color'],
		'link' => $cmsmasters_option['mall_default_link'],
		'hover' => $cmsmasters_option['mall_default_hover'],
		'heading' => $cmsmasters_option['mall_default_heading'],
		'bg' => $cmsmasters_option['mall_default_bg'],
		'alternate' => $cmsmasters_option['mall_default_alternate'],
		'border' => $cmsmasters_option['mall_default_border'],
	);
...
function cmsmasters_tribe_additional_template_locations( string $file, string $template ) {
	$new_locations = array();

	if ( ! tribe_events_views_v2_is_enabled() ) {
		$new_locations['themes_root'] = trailingslashit( get_theme_root() ) . 'mall/tribe-events';
	}
...
add_filter( 'tribe_events_template', 'cmsmasters_tribe_additional_template_locations', 10, 2 );


/* Replace Styles */
function cmsmasters_tribe_events_stylesheet_url() {
	if ( ! tribe_events_views_v2_is_enabled() ) {
...
add_action('wp_enqueue_scripts', 'cmsmasters_tribe_events_stylesheet_url', 100);


/* Replace Pro Styles */
function cmsmasters_tribe_events_pro_stylesheet_url() {
	if ( ! tribe_events_views_v2_is_enabled() ) {
...
add_action('wp_enqueue_scripts', 'cmsmasters_tribe_events_pro_stylesheet_url', 100);


/* Replace Widget Styles */
function cmsmasters_tribe_events_pro_widget_calendar_stylesheet_url() {
	if ( ! tribe_events_views_v2_is_enabled() ) {
		return '';
	}
}

add_filter('tribe_events_pro_widget_calendar_stylesheet_url', 'cmsmasters_tribe_events_pro_widget_calendar_stylesheet_url');


/* Replace Responsive Styles */
function cmsmasters_tribe_events_mobile_breakpoint() {
	if ( ! tribe_events_views_v2_is_enabled() ) {
		return 768;
	}
}

add_filter('tribe_events_mobile_breakpoint', 'cmsmasters_tribe_events_mobile_breakpoint');
